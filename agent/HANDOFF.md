...
| docs/use-cases/sequences/ | Sequence diagram 3 lớp tạo bằng Archify, mỗi use case một folder riêng |
...
node C:\Users\PC\.codex\skills\archify\bin\archify.mjs validate sequence docs/use-cases/sequences/uc01_usecase_dang_nhap/diagram.sequence.json --quality showcase --json
node C:\Users\PC\.codex\skills\archify\bin\archify.mjs deliver sequence docs/use-cases/sequences/uc01_usecase_dang_nhap/diagram.sequence.json docs/use-cases/sequences/uc01_usecase_dang_nhap/diagram.html --quality showcase --json
```

Kết quả: typecheck/lint/build đạt; 84/85 test đạt, 1 bỏ qua vì thiếu vector thật, 0 lỗi. Hai migration đã áp dụng thành công trên DB thử nghiệm local riêng. Bộ quyền có 16 test tích hợp SQL/Prisma/service/HTTP. Build frontend lần đầu bị chặn tải font, chạy lại có quyền mạng đã đạt. Sau đổi tên sản phẩm, đã chạy lại generator, reload trang use case trong trình duyệt và chạy `corepack pnpm --filter @tang-thu/web typecheck` đạt. Sequence diagram từng use case validate/deliver bằng Archify đạt cho 19 folder. Chưa chạy browser end-to-end trên ứng dụng.
...
Test độc lập: DB tangthu_cntt_test_20260905 đã tồn tại trong tang-thu-db-1. Chạy node server/scripts/test-cntt-db.mjs từ gốc repo. Không gọi lại createdb nếu DB đã có. Xem docs/use-cases/tang-thu-use-case.html và docs/use-cases/sequences/index.html; server preview tạm dùng cổng 8768.
